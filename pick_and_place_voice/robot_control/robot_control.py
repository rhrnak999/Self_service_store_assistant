...
# 결제 관련 좌표 (필요에 따라 수정)
PAYMENT_POS = [-29.62, -3.3, 99.49, -0.33, 83.61, 5.74]  # 결제 위치
REFUND_POS = [-84.21, 11.79, 103.71, 0.1, 64.51, 5.74]   # 환불 위치
...
class RobotController(Node):
    def __init__(self):
        ...
        # PaymentCommand 서비스 서버 추가
        self.payment_service = self.create_service(
            PaymentCommand,
            'payment_command',
            self.payment_command_callback
        )
        ...
        self.get_logger().info("PaymentCommand 서비스 서버 시작됨")
        
        # 로봇 초기화
        self.init_robot()

    #####################
    # PaymentCommand 서비스 콜백 (새로 추가)
    #####################
    def payment_command_callback(self, request, response):
        """PaymentCommand 서비스 콜백 함수"""
        command = request.command.lower().strip()
        
        self.get_logger().info(f"결제 명령 수신: {command}")
        
        if self.robot_busy:
            response.success = False
            response.message = "로봇이 다른 작업을 수행 중입니다. 잠시 후 다시 시도해주세요."
            return response
        ...
        try:
            ...
            if command == "pay":
                success = self.handle_payment_request()
                if success:
                    response.success = True
                    response.message = "결제 처리가 완료되었습니다."
                else:
                    response.success = False
                    response.message = "결제 처리 중 오류가 발생했습니다."
                    
            elif command == "refund":
                success = self.handle_refund_request()
                if success:
                    response.success = True
                    response.message = "환불 처리가 완료되었습니다."
                else:
                    response.success = False
                    response.message = "환불 처리 중 오류가 발생했습니다."
                    
            else:
                response.success = False
                response.message = f"알 수 없는 명령입니다: {command}. 'pay' 또는 'refund'를 사용하세요."
                
        except Exception as e:
            response.success = False
            response.message = f"서비스 처리 중 오류 발생: {str(e)}"
            self.get_logger().error(f"PaymentCommand 서비스 오류: {str(e)}")
        finally:
            self.robot_busy = False
        
        self.get_logger().info(f"PaymentCommand 응답: {response.message}")
        return response

    def handle_payment_request(self):
        """결제 요청 처리 함수 - 사용자가 구현할 예정"""
        try:
            self.get_logger().info("결제 처리를 시작합니다...")
            
            # TODO: 여기에 결제 관련 로봇 움직임 구현
            # 예시 구조:
            # 1. 결제 위치로 이동
            # 2. 결제 동작 수행
            # 3. 바코드 확인 등
            ...
            # 임시 구현 (사용자가 수정할 예정)
            self.get_logger().info("결제 동작 수행 중... (구현 필요)")
            time.sleep(2)  # 임시 대기
            
            # 홈 위치로 복귀
            self.return_to_home()
            
            self.get_logger().info("결제 처리 완료")
            return True
            
        except Exception as e:
            self.get_logger().error(f"결제 처리 중 오류: {str(e)}")
            self.return_to_home()
            return False

    def handle_refund_request(self):
        """환불 요청 처리 함수 - 사용자가 구현할 예정"""
        try:
            self.get_logger().info("환불 처리를 시작합니다...")
            
            # TODO: 여기에 환불 관련 로봇 움직임 구현
            # 예시 구조:
            # 1. 환불 위치로 이동
            # 2. 환불 동작 수행
            # 3. 처리 완료 확인 등
            ...
            # 임시 구현 (사용자가 수정할 예정)
            self.get_logger().info("환불 동작 수행 중... (구현 필요)")
            time.sleep(2)  # 임시 대기
            
            # 홈 위치로 복귀
            self.return_to_home()
            
            self.get_logger().info("환불 처리 완료")
            return True
            
        except Exception as e:
            self.get_logger().error(f"환불 처리 중 오류: {str(e)}")
            self.return_to_home()
            return False
    ...
    def return_to_home(self):
        """홈 위치로 복귀"""
        try:
            self.get_logger().info("홈 위치로 복귀 중...")
            self.init_robot()
            self.get_logger().info("홈 위치 복귀 완료")
        except Exception as e:
            self.get_logger().error(f"홈 복귀 오류: {str(e)}")
    ...
    def robot_control(self):
        """음성 명령 기반 픽앤플레이스 제어"""
        if self.robot_busy:
            self.get_logger().info("로봇이 결제/환불 작업 중입니다. 대기 중...")
            return
        ...
        try:
            ...
            if get_keyword_future.result().success:
                ...
                self.get_logger().warn("키워드 서비스 호출 실패")
                return
        except Exception as e:
            self.get_logger().error(f"키워드 서비스 오류: {str(e)}")
            return
        ...
        for target, count in zip(object_list, count_list):
            for _ in range(count):
                if self.robot_busy:  # 결제/환불 요청이 들어오면 중단
                    self.get_logger().info("결제/환불 요청으로 인해 픽앤플레이스 중단")
                    return
    ...
    def robot_control_loop():
        ...
        while rclpy.ok():
            try:
                ...
                node.get_logger().error(f"Robot control loop error: {str(e)}")
                time.sleep(5)  # 오류 시 5초 대기
    
    # 로봇 제어 스레드 시작
    robot_thread = Thread(target=robot_control_loop)
    robot_thread.daemon = True
    robot_thread.start()
    
    try:
        node.get_logger().info("Robot Controller 시작 (MultiThread)")
        node.get_logger().info("PaymentCommand 서비스 대기 중...")
        executor.spin()  # PaymentCommand 서비스 요청 대기
    except KeyboardInterrupt:
